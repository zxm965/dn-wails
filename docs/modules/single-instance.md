...
- `internal/platform/singleinstance/configure.go`：按构建模式配置 Wails `SingleInstanceLock` 和固定 UUID。
...
```text
用户再次启动应用
  → 非开发构建启用 SingleInstanceLock
  → Wails SingleInstanceLock
  → application.SecondInstanceHandler
  → singleinstance.Service.Normalize
  → windowService.Activate
  → EventsEmit("app:second-instance")
  → useAppLifecycle
```
...
- DOM 未就绪时不会直接发送前端事件，而是进入待处理队列。
- 本模块只转发参数，不直接把参数当作文件路径、URL 或命令执行。
- `wails dev` 使用 `dev` 构建标签；该模式不启用单实例锁，避免 Go 文件重建后的新进程被旧进程识别为第二实例。
...
Go 后端热重载需要在修改生效后重新启动一次现有 `wails dev`，后续 Go 文件更新即可由 Wails 自动重建并替换进程。
