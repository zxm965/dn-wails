...
- 桌面框架：Wails v3（当前锁定 `v3.0.0-beta.16`）。
- 后端：Go。
...
- 当用户输入“发布更新”，或明确要求提交当前代码、推送远端并发布最新版时，按以下一次性流程执行，不需要再次确认版本号：
  1. 先检查工作区、当前分支、远程仓库和最新稳定版本标签；保留当前工作区内属于用户的全部改动，不回退或清理无关文件。
  2. 根据改动范围执行必要的格式检查、测试、lint 和构建；不得因为发布而启动本地服务或桌面应用。
  3. 将当前代码改动提交，提交信息使用简洁且能概括本次变更的 Conventional Commit 风格文本。
  4. 将当前分支推送到 `origin`；默认沿用项目当前分支，不创建额外发布分支。
  5. 在最新稳定 `vMAJOR.MINOR.PATCH` 标签基础上递增 PATCH 创建下一个版本标签，并推送该标签；若目标标签已存在，停止并报告冲突，不覆盖已有标签。
  6. 标签推送用于触发现有发布工作流；完成标签推送后立即结束本次发布操作，除非用户明确要求，否则不查询、不轮询、不监控 CI 或 Release 发布状态。
- 最终回复必须说明提交哈希、推送分支、发布标签、已执行的验证，以及未监控后续发布任务这一点。
